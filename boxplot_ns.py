...
axs[0].text(x + 7*step, y, 'p.b.', transform=axs[0].transAxes, fontsize=fontje, va='top')
...
def box_lumped_or_distributed(lumped): 
    ...
    for variable in variables:
        ...
        axs[i].get_xaxis().set_ticks([])
        axs[i].text(-0.21,0.5,names[i],transform=axs[i].transAxes, va='center', ha='right')
        i += 1

    plt.xticks([*range(1,9)], ["E", "S", "G", "ES", "EG", "SG", "ESG", "p.b."])
    #fig.set_size_inches(8.27, 11.69)
    fig.set_size_inches(5.27, 11.69)
